            ...
            <div class="card-body">
                <form action="{{ URL::to('/admin/time/store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row border-bottom pb-4 pb-lg-0">
                        <div class="col-12 col-md-8 col-xl-6 mb-lg-4 md-1">
                            <label class="form-label">
                                ...
                                <div class="d-flex">
                                    <input type="text" class="form-control w-50 time-interval {{session()->get('direction') == 2 ? 'rounded-start-0 rounded-end-5' : 'rounded-end-0 rounded-start-5'}}" name="interval_time"
                                        placeholder="{{ trans('labels.time_interval') }}" aria-describedby="button-addon2"
                                        value="{{$settingsdata->interval_time}}" required>
                                    <select name="interval_type" class="form-select w-25 working-hoursselect {{session()->get('direction') == 2 ? 'rounded-start-5 rounded-end-0' : 'rounded-end-5 rounded-start-0'}}" id="">
                    ...
                    <div class="row pt-2 pt-md-3">
                        ...
                        <label
                            class="col-md-2 text-satrt mb-3 d-none d-lg-block d-xl-block d-xxl-block fw-500 fs-6">{{ trans('labels.opening_time') }}</label>
                        <label
                            class="col-md-2 text-satrt mb-3 d-none d-lg-block d-xl-block d-xxl-block fw-500 fs-6">{{ trans('labels.break_start') }}</label>
                        <label
                            class="col-md-2 text-satrt mb-3 d-none d-lg-block d-xl-block d-xxl-block fw-500 fs-6">{{ trans('labels.break_end') }}</label>
                        <label
                            class="col-md-2 text-satrt mb-3 d-none d-lg-block d-xl-block d-xxl-block fw-500 fs-6">{{ trans('labels.closing_time') }}</label>
                        <label
                            class="col-md-2 text-satrt mb-3 d-none d-lg-block d-xl-block d-xxl-block fw-500 fs-6">{{ trans('labels.always_closed') }}</label>
                    </div>
                    ...
                    <div class="form-group m-0 mt-2 d-flex gap-2 justify-content-end">
                        <button
                            ...
                            class="btn btn-secondary px-4 rounded-start-5 rounded-end-5 {{ Auth::user()->type == 4 ? (helper::check_access('role_working_hours', Auth::user()->role_id, $vendor_id, 'add') == 1 ? '' : 'd-none') : '' }}">{{ trans('labels.save') }}</button>
                    </div>
                </form>
            </div>
        </div>
